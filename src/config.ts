...
export const config = {
  ...
  calibration: {
    ...
    blinkStepMs: 5500,
    closedStepMs: 2000,
    closeWaitMs: 5000, // ждать сомкнутых век не дольше (дальше — по таймеру, как раньше)
    closeDelta: 0.2, // M0: сырой eyeBlink выше открытых на столько — «закрыты» (только для шага калибровки)
  ...
  // Силуэт (GDD → Угроза): двигается, только когда на него не смотрят.
  stalker: {
    spawnMinDist: 70, // м от маяка
    speed: 1.5, // м/с, выслеживание (игрок идёт 2.2)
    approachSpeed: 1.8,
    huntSpeed: 2.5, // финальная охота: пешком не уйти, только бегом
    retreatSpeed: 3.2,
    creepMul: 0.3, // в поле зрения, но взгляд не на нём
    closedMul: 1.8, // пока глаза закрыты
    heightMul: 1.1, // выше экспедиции — примета
    armMul: 1.25, // руки длиннее — примета
    blinkLeap: 1.2, // м, рывок на моргании
    safeDist: 4, // рывок не подводит ближе (моргание не убивает)
    killDist: 1.1,
    holdMaxMs: 4000, // держать взглядом не дольше, потом он медленно идёт
    holdResetMs: 2000, // отвёл взгляд на столько — «привыкание» сброшено
    gazeHoldAngle: 0.21, // рад (~12°) между взглядом и направлением на него
    viewHalfAngle: 0.9, // рад: в этом конусе от направления камеры он «на виду»
    hearing: 90, // м: слышит импульс
    hearRun: 14, // м: слышит бег
    revealDist: 9, // м: скан поймал его ближе — удар и передышка
    replanMs: [4500, 9000] as [number, number],
    coverRadius: 0.45, // м за стволом
  },

  // Директор напряжения (как в Left 4 Dead / Alien: Isolation): нагнетание → пик → передышка.
  director: {
    base: 0.006, // в секунду
    study: 0.014, // пока идёт детальный скан
    closed: 0.01, // пока глаза закрыты
    near: 0.02, // пока он ближе nearDist
    nearDist: 15,
    perSpecies: 0.08, // скачок за описанный вид
    perPulse: 0.03, // импульс шумит
    relaxMs: 22_000, // передышка после близкого контакта
    relaxDecay: 0.04, // в секунду
    approachAt: 0.7, // напряжение, с которого он идёт вплотную
    studyApproachAt: 0.45, // …или ниже, если ты занят образцом
    stillMs: 12_000, // стоишь дольше — подходит
    graceMs: 25_000, // начало забега — только издалека
    farDist: 42, // м, дистанция выслеживания при нулевом напряжении
    nearDist2: 14, // м, при максимальном
  },

  // Застывшая экспедиция: статуи-люди с записями.
  expedition: {
    statues: 6,
    minDist: 20,
    maxDist: 85,
    reach: 1.8, // м: прочитать запись
    hintMs: 90_000, // метка вида на компасе после записи-подсказки
  },
